...
class Main extends React.Component {
  constructor(props) {
    super(props);
    this.state = {sites: props.sites, options: props.options, isLoadStarted: false};
  }
  ...
  render() {
    return(
      <main>
        <SettingPanel sites={this.state.sites} options={this.state.options}
          updateSites={(sites) => this.updateSites(sites)} />
        <FrameContainer ref="container" sites={this.state.sites} updateSites={(sites) => this.updateSites(sites)} />
        <PauseLayer isVisible={this.state.isLoadStarted !== true} startLoad={() => {this.refs.container.startLoad()}} />
      </main>
    );
  }
}
...
class SettingPanel extends React.Component {
  constructor(props) {
    ...
    this.state = {sites: props.sites || [], options: props.options};
  }
  ...
  render() {
    return(
      <aside className="setting">
        <ul>
          <li>
            <SiteAdditionForm sites={this.state.sites} options={this.state.options}
              updateSites={(sites) => this.updateSites(sites)} />
          </li>
        </ul>
      </aside>
...
class SiteAdditionForm extends React.Component {
  constructor(props) {
    ...
    this.updateSites = props.updateSites;
    this.state = {sites: props.sites || [], options: props.options, newSiteUrl: ''};
  }
  ...
  render() {
    ...
    return (
      <form action="#" onSubmit={(e) => {this.addNewSite(e)}}>
        <input type="url" value={this.state.newSiteUrl} onChange={(e) => {this.editNewSiteUrl(e)}}
...
class FrameContainer extends React.Component {
  constructor(props) {
    ...
    this.updateSites = props.updateSites;
    this.state = {sites: props.sites || [], isLoadStarted: false};
  }
